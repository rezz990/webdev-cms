...
class PublicSiteController extends Controller
{
    ...
    public function project(Project $project): View { abort_unless(Project::published()->whereKey($project)->exists(),404); $project->load(['category','technologies','images']); return view('public.projects.show',['project'=>$project,'related'=>Project::published()->whereKeyNot($project)->limit(3)->get()]); }
    ...
    public function post(Post $post): View { abort_unless(Post::published()->whereKey($post)->exists(),404); $post->load(['category','tags','author']); return view('public.blog.show',['post'=>$post,'related'=>Post::published()->whereKeyNot($post)->limit(3)->get()]); }
    ...
    public function storeContact(ContactRequest $request): RedirectResponse { ContactMessage::create($request->safe()->except('website')); return back()->with('success','Pesan sudah tersimpan. Terima kasih sudah menghubungi Reza.'); }
